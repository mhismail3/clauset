...
pub struct SessionActivity {
    ...
    /// Whether the terminal status line included a context percentage.
    pub status_line_context_seen: bool,
}
...
impl Default for SessionActivity {
    fn default() -> Self {
        Self {
            ...
            status_line_context_seen: false,
        }
    }
}
...
impl SessionBuffers {
    ...
    fn parse_and_update_activity(&self, buffer: &mut TerminalBuffer, new_chunk: &str, full_buffer: &str) -> bool {
        ...
        if let Some(status) = parse_status_line(&clean_buffer) {
            ...
            let ctx_changed = status.context_percent
                .map(|ctx| buffer.activity.context_percent != ctx)
                .unwrap_or(false);

            if input_changed || output_changed || ctx_changed {
                if let Some(ctx) = status.context_percent {
                    tracing::debug!(
                        target: "clauset::activity::stats",
                        "Stats updated (status line): tokens={}K/{}K, ctx={}%",
                        status.input_tokens/1000, status.output_tokens/1000, ctx
                    );
                } else {
                    tracing::debug!(
                        target: "clauset::activity::stats",
                        "Stats updated (status line): tokens={}K/{}K, ctx=unknown",
                        status.input_tokens/1000, status.output_tokens/1000
                    );
                }
                buffer.activity.input_tokens = status.input_tokens;
                buffer.activity.output_tokens = status.output_tokens;
                if let Some(ctx) = status.context_percent {
                    buffer.activity.context_percent = ctx;
                }
            ...
            if status.context_percent.is_some() {
                if !buffer.activity.status_line_context_seen {
                    buffer.activity.status_line_context_seen = true;
                    changed = true;
                }
            } else if buffer.activity.status_line_context_seen {
    ...
    pub async fn update_context_from_hook(
        ...
        let mut changed = false;
        let use_status_line_tokens = buffer.activity.status_line_seen;
        let use_status_line_context = buffer.activity.status_line_context_seen;
        ...
        if effective_window > 0 && !use_status_line_context {
    ...
    ///
    /// For display, we keep billed token totals separate from cache tokens:
    /// - Input/Output totals accumulate per-message usage (excluding cache)
    /// - Cache tokens are tracked separately for context calculations
    pub async fn accumulate_usage(
        ...
        // Transcript usage input tokens exclude cached context; keep cache tokens separate.
        if !buffer.activity.hook_context_received && !use_status_line_tokens {
            buffer.activity.input_tokens += input_tokens;
            buffer.activity.output_tokens += output_tokens;
        }
        ...
        // Recalculate context percentage using the current message's context usage.
        if buffer.activity.context_window_size > 0
            && !buffer.activity.hook_context_received
            && !use_status_line_context
        {
            let current_context = input_tokens + cache_read_tokens + cache_creation_tokens;
            buffer.activity.context_percent =
                ((current_context.saturating_mul(100)) / buffer.activity.context_window_size)
                    .min(100) as u8;
        }
...
mod tests {
    ...
    proptest! {
        ...
        fn prop_valid_status_lines_parse(
            ...
            if let Some(parsed) = parse_status_line(&line) {
                ...
                prop_assert_eq!(parsed.context_percent, Some(ctx));
            }
        }
    ...
    fn test_parse_status_line() {
        ...
        assert_eq!(status.context_percent, Some(11));
    }
    ...
    fn test_parse_status_line_partial() {
        ...
        assert_eq!(status2.context_percent, None);
        ...
        assert_eq!(status3.context_percent, Some(5));
    }
    ...
    fn test_parse_status_line_without_k_suffix() {
        ...
        assert_eq!(status.context_percent, Some(1));
    }
    ...
    fn test_parse_status_line_m_suffix() {
        ...
        assert_eq!(status.context_percent, Some(80));
    }
    ...
    fn test_parse_status_line_multiline() {
        ...
        assert_eq!(status.context_percent, Some(21));
    ...
    fn test_parse_status_line_ctx_on_separate_line() {
        ...
        assert_eq!(status.context_percent, Some(19));
    }

    #[test]
    fn test_parse_status_line_ctx_wrapped_after_full_line() {
        let wrapped = "Haiku 4.5 | $0.08 | 3.9K/1.5K |\nctx:19%";
        let status = parse_status_line(wrapped).unwrap();
        assert_eq!(status.model, "Haiku 4.5");
        assert!((status.cost - 0.08).abs() < 0.01);
        assert_eq!(status.input_tokens, 3900);
    ...
    fn test_parse_status_line_update_available_multiline() {
        ...
        assert_eq!(status2.context_percent, Some(21));
    }
    ...
    async fn test_transcript_context_percent_uses_current_message() {
        ...
        let activity = buffers
            .accumulate_usage(session_id, 1000, 100, 9000, 0, "claude-sonnet-4-5")
            .await
            .unwrap();

        assert_eq!(activity.context_percent, 5);

        let activity2 = buffers
            .accumulate_usage(session_id, 1000, 100, 9000, 0, "claude-sonnet-4-5")
            .await
            .unwrap();

        assert_eq!(activity2.context_percent, 5);
        assert_eq!(activity2.input_tokens, 2000);
